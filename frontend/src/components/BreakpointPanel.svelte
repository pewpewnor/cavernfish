<script lang="ts">
  ...
  $: if (activeHit) {
    editingResponse = JSON.parse(JSON.stringify(activeHit.response))
  }

  async function release() {
    if (!activeHit || !editingResponse) return
    await api.releaseBreakpoint(activeHit.id, editingResponse)
    pendingBreakpoints.update((list) => list.filter((b) => b.id !== activeHit!.id))
    activeIdx = Math.min(activeIdx, $pendingBreakpoints.length - 1)
  }

  async function discard() {
    if (!activeHit) return
    await api.discardBreakpoint(activeHit.id)
    pendingBreakpoints.update((list) => list.filter((b) => b.id !== activeHit!.id))
    activeIdx = Math.min(activeIdx, $pendingBreakpoints.length - 1)
  }
...
>
  <div
    class="flex h-[620px] max-h-[90vh] w-[900px] max-w-[95vw] flex-col overflow-hidden rounded-lg border border-accent bg-cave-elevated shadow-bp"
  >
    <!-- Header -->
    <div class="flex shrink-0 items-center gap-3 border-b border-wire bg-cave-surface px-4 py-3">
      ...
      <div class="ml-auto shrink-0">
        {#if $pendingBreakpoints.length > 1}
          <button class="btn btn-ghost text-[11px]" on:click={releaseAll}>Release All</button>
        {/if}
      </div>
    </div>
    ...
    {#if activeHit}
        ...
        <div class="flex flex-col overflow-hidden border-r border-wire">
          ...
          >
            ...
            <div class="flex items-center gap-2">
              <span
                ...
                style="background:{mc(activeHit.method)};color:#050508">{activeHit.method}</span
              >
          ...
          <div class="flex flex-1 flex-col overflow-y-auto p-2.5">
            {#if reqTab === 'body'}
              ...
              {:else}
                <div class="py-10 text-center text-xs text-ink-muted">No request body</div>
              {/if}
            {:else}
              <pre
                class="whitespace-pre-wrap break-all font-mono text-[11px] leading-relaxed text-ink-mid">{formatHeaders(
        ...
        <div class="flex flex-col overflow-hidden">
          ...
          >
            ...
            <span
              ...
              style="color:{statusColor(editingResponse?.statusCode ?? 200)}"
          ...
          {#if editingResponse}
            <div class="flex flex-1 flex-col overflow-hidden">
              <ResponseEditor bind:response={editingResponse} />
            </div>
          {/if}
        </div>
      </div>
      ...
      >
        ...
        <div class="flex gap-2">
          ...
          <button class="btn btn-primary px-4 py-1.5 text-[13px]" on:click={release}>
